...
<div class="py-8 min-h-screen">
    ...
    <div class="flex justify-between flex-wrap gap-2">
        @foreach ($categories as $k)
            <a href="{{ route('produk.index', $k->nama) }}" class="py-3 px-4 w-50 text-center uppercase rounded-md shadow-md font-bold tracking-widest cursor-pointer {{ $selectedCategories === $k->nama ? 'bg-accent text-white' : 'bg-white text-accent' }}">{{ $k->nama }}</a>
        @endforeach
    </div>
    <br>
    <div>
            ...
            <div class="p-4 md:p-5 space-y-4">
                <div class="border rounded overflow-hidden overflow-y-auto max-h-64">
                    <table class="table-auto w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            <tr class="border-t">
                                <td class="px-4 py-2">{/{ $item->id_produk }}</td>
